...
BEGIN
	...
	UPDATE cliente SET ape_pat = UPPER(ape_pat);    
	UPDATE cliente SET ape_mat = UPPER(ape_mat);    
	UPDATE cliente SET direccion = UPPER(direccion);    
	UPDATE cliente SET cel = UPPER(cel);
	UPDATE cliente SET correo = UPPER(correo);
...
BEGIN
	...
	UPDATE empleado SET nombre = UPPER(nombre);    
	UPDATE empleado SET ape_pat = UPPER(ape_pat);
	UPDATE empleado SET ape_mat = UPPER(ape_mat);
	UPDATE empleado SET telefono = UPPER(telefono);
	UPDATE empleado SET direccion = UPPER(direccio);
	UPDATE empleado SET email = UPPER(email);
	UPDATE empleado SET rfc = UPPER(rfc);
...
BEGIN
	...
	UPDATE material SET descripcion = UPPER(descripcion);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
	...
	INSERT INTO prospecto (nombre,ape_pat,ape_mat,cel,correo,doc) 
    values (nom,a_p,a_m,cel,correo,doc);
	UPDATE prospecto SET nombre = UPPER(nombre);
	UPDATE prospecto SET ape_pat = UPPER(ape_pat);
    UPDATE prospecto SET ape_mat = UPPER(ape_mat);
    UPDATE prospecto SET cel = UPPER(cel);
    UPDATE prospecto SET correo = UPPER(correo);
    UPDATE prospecto SET doc = UPPER(doc);
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2018-11-28 12:27:47
